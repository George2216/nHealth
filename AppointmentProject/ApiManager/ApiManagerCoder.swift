...
class XMLMultiCoder<Output:Codable> {
    ...
    func parsData<Input:Codable>(input:Input?,metod:Metod ,phpFunc:PHPFunc, ecodeParam:EncodeParam) -> Observable<(Output?,String?)> {
        return Observable<(Output?,String?)>.create { observable in

            let url = URL(string: "http://82.117.240.50/toothfairyhost/optimal/toothfairy/services/api/APIDocUA.php?func=\(phpFunc.rawValue)")!
