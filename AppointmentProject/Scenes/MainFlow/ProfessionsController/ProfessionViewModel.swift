...
extension ProfessionViewModel {
    private var specialtiesDriver:Driver<[SpecialitysUI]> {
        return  professions.asDriverOnErrorJustComplete()
    }
    private var cellIsEnubleDriver:Driver<Bool> {
        return  cellIsEnuble.asDriverOnErrorJustComplete()
    }
    private var saveDataDriver: Driver<(clinicId:String,professions:[String])> {
        return saveData.asDriverOnErrorJustComplete()
    }
    
}

final class ProfessionViewModel: ViewModelProtocol {
    ...
    func transform(_ input: Input) -> Output {
        ...
        return Output(specialties: specialtiesDriver, cellIsEnuble: cellIsEnubleDriver, saveData: saveDataDriver, goBack: input.tapXmark.asDriver(onErrorJustReturn: ()))
    }
    ...
    private func getProfession(_ input:Input) {
        input.idClinic.subscribe(onNext: { [self] id in
            ...
            apiManager.parsData(input: InputModelDoctors(Token: SingletonData.shared.token, CenterId: id), metod: .POST, phpFunc: .SpecialtyList, ecodeParam: EncodeParam(withRootKey: "Root", rootAttributes: nil, header: nil)).asObservable().subscribe(onNext: { model , _  in
