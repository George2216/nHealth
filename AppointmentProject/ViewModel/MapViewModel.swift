...
extension MapViewModel {
    private var latitudeDriver:Driver<Double> {
        return latitude.asDriver(onErrorJustReturn: 0)
    }
    private var longitudeDriver:Driver<Double> {
        return longitude.asDriver(onErrorJustReturn: 0)
    }
    private var titleDriver:Driver<String> {
        return title.asDriver(onErrorJustReturn: "")
    }
    private var subtitleDriver:Driver<String> {
        return subtitle.asDriver(onErrorJustReturn: "")
    }
    private var titleText:Driver<String> {
        return Localizable.localize(.map).asDriver(onErrorJustReturn: "")
    }
